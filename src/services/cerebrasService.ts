...
import { SwarmResponse } from '../types';

// Inicializar el cliente oficial del SDK de Cerebras
const client = new Cerebras({
  apiKey: process.env.CEREBRAS_API_KEY || '',
});

/**
 * Ejecuta el pipeline secuencial del enjambre multi-agente en Cerebras Cloud.
 * Agente 1: Perceptor (Visión Multimodal, razonamiento desactivado)
 * Agente 2: Estratega (Razonamiento lógico y estructurado de decisiones de ruta)
 *
 * @param image Imagen en formato Base64 URI
 * @param robotPos Posición actual del robot [x, y]
 * @param gridSize Tamaño del entorno de simulación
 * @param history Historial de últimos movimientos
 * @returns Promesa con la respuesta del enjambre y métricas detalladas
 */
export async function runSwarmPipeline(
  ...
  const currentRobotCoords = `[${robotPos[0]}, ${robotPos[1]}]`;
  const goalCoords = `[${maxCoord}, ${maxCoord}]`;
  ...
  const perceptorResponse = await client.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: `Eres el sistema visual de un robot autónomo. Analiza la imagen matricial adjunta (que representa un lienzo de simulación de una grilla de ${gridSize}x${gridSize}).
La posición exacta actual del robot en la grilla es ${currentRobotCoords}.
Identifica las coordenadas (X, Y) exactas (indexadas de 0 a ${maxCoord}) de:
1. El Robot (que se encuentra exactamente en ${currentRobotCoords}).
2. La Meta (indicada por el cuadro verde neón, en ${goalCoords}).
...
Genera un reporte conciso y resumido en formato textual plano con las coordenadas detectadas para alimentar al agente estratega.`,
    ...
    // reasoning_effort desactivado para optimizar TTFT en percepción visual
  ...
  // --- AGENTE 2: ESTRATEGA (PENSAMIENTO LÓGICO Y ESTRUCTURADO) ---
...
Basándote en estos datos, calcula analíticamente la trayectoria más rápida y segura para que el robot avance hacia la meta ${goalCoords} evadiendo los obstáculos detectados.
...
${perceptorReport}`,
  ...
  let estrategaDecision = {
    next_move: '' as 'UP' | 'DOWN' | 'LEFT' | 'RIGHT' | '',
  ...
  try {
    ...
    console.error('Error parsing estratega structured output in service:', parseErr);
  }
